...
//      Rcout << "bscale pars " << " " << 1/ww0 << " " << 1/ww1 << endl;
  ...
  if(use_mscale) {
    ...
    if( ((i>=burn) & (i % thin==0)) )  {
      ...
      for(size_t k=0;k<n;k++) {
        post_mu(save_ctr, k) = allfit_con[k];
  ...
  return(List::create(_["yhat_post"] = yhat_post, _["post_mu"] = post_mu, _["b_post"] = b_post, _["b_est_post"] = b_est_post,  
